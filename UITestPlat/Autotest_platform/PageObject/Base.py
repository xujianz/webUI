...
class PageObject:
        ...
        try:
            try:
                if more:
                    ...
                    return WebDriverWait(driver, timeout).until(ec.visibility_of_element_located(locator))#‘’‘判断某个元素是否被添加到了dom里并且可见，可见代表元素可显示且宽和高都大于0’’’
            except:
                if more:
                    return WebDriverWait(driver, timeout).until(ec.presence_of_all_elements_located(locator))
                else:
                    return WebDriverWait(driver, timeout).until(ec.presence_of_element_located(locator))#‘’‘判断某个元素是否被加到了dom树里，并不代表该元素一定可见，如果定位到就返回WebElement’’’
        except Exception:
    ...
    按钮开关
    '''
    def openOrcloseButton(self,locator,operate):
        el = PageObject.find_element(self.driver, locator)
        classlist = el.get_attribute('class')
        state = '2'
        if 'is-checked' in classlist:
            state='1'
        try:
            if operate!=state:
                self.click(locator)
        except:
            raise ValueError("开关参数：" + operate+"不符合要求")
