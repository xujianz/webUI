        ...
        for bl in beforeLogin:
            ...
            for loginStep in loginSteps:
                try:
                    ...
                    split.finishTime = timezone.now()
                    # 截图，保存图片信息
                    picname = time.strftime("%Y%m%d%H%M%S", time.localtime(time.time())) + '.png'
                    path=os.path.join(BASE_DIR, 'media\\')+picname
                    driver.get_screenshot_as_file(path)
                    split.screenshot=picname
                    split.save()
                    if driver:
                        driver.quit()
                    return
...
class Step:
    ...
    def sys_method__run(self, driver, value):
        '''
        keyword.package->Autotest_platform.PageObject.Base 包
        keyword.clazz->相关的类PageObject
        :param driver:浏览器驱动
        :param value:
        :return:
        '''
        package = __import__(self.keyword.package, fromlist=True)  # 导入包(模块)
        clazz = getattr(package, self.keyword.clazz)  # 获取模块中的类
        setattr(clazz, "driver", driver)  # 给包中类的添加类的属性
        method = getattr(clazz, self.keyword.method)  # 获取类的方法

        # 执行操作浏览器驱动的函数
        def running(*args):
            try:
                c = clazz()  # 创建一个类对象PageObject
                para = (c,)  # tuple，c代表类本身self
                args = para + args[0]  # 所以这里的args=（self,外部传入的参数）
                method(*args)  # 获取对象属性值后直接使用。举例说明：如果是self.keyword.method=click那么method(*args)=click(self,locator)
            except:
                raise
