...
import os
from Autotest_platform.settings import BASE_DIR
...
class Project:
    ...
    def create(request):
        #新增项目
        try:
            parameter = get_request_body(request)# 获取请求参数body
        except ValueError:
            return JsonResponse.BadRequest("json格式错误")
        p = project()#创建一个model对象，操作数据库
        p.creator = request.user.username#项目创建者
        p.name = parameter.get("name", "")#项目名，parameter是一个字典，字典中的get()函数，如果name的value不存在，则返回定义的“”
        p.remark = parameter.get("remark", "")#备注
        try:
            p.clean()
        ...
        p.name = p.name.strip()
        # 判断项目名是否已存在
        if project.objects.filter(name__exact=p.name):#相当于sql语句中的条件查询where
    ...
    def delete(request, project_id):
        '''项目管理-删除项目'''
        p = get_model(project, id=project_id)#根据项目id查询
        if not p:
    ...
    def edit(request, project_id):
        ...
        try:
            p.clean()#判断项目名称是否符合限制条件，字符长度0-20
        except ValidationError as ve:
            return JsonResponse.BadRequest(','.join(ve.messages))
        # 判断重复
        p.name = p.name.strip()
...
class Element:
    ...
    def find(request):
        ...
        page_index = request.GET.get("p", 1)
        page_index = int(page_index) if str(page_index).isdigit() and int(page_index) >= 1 else 1
        page_size = parameter.get("pageSize", 50)
        page_size = int(page_size) if str(page_size).isdigit() and int(page_size) >= 1 else 10
        start_time = "2018-01-01 00:00:00"
        start_time = parameter.get("startTime", start_time) if parameter.get("startTime", start_time) else start_time
        now = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
...
class TestResult:
    ...
    def get(request, result_id):
        ...
        splitResult = list()
        for s in split:
            sd = model_to_dict(s, ["status", 'expect', 'remark','screenshot'])
    ...
    @staticmethod
    def get_img(request,result_id):
        try:
            re = get_model(Result, id=result_id)
            result = model_to_dict(re, ["id"])
            from .models import SplitResult
            split = get_model(SplitResult, False, resultId=re.id)
            if not split:
                return JsonResponse.BadRequest("该测试结果不存在")
            ...
            return JsonResponse.BadRequest("json格式错误")
        imgpath=os.path.join(BASE_DIR,'media\\screenshot\\')
        imagepath = imgpath+splitResult[0]['screenshot']
...
class TestTasks:
    ...
    def create(request):
        ...
        try:
            print(t.name,t.remark,t.testcases)
            print(t.timing)
            print(t.browsers)
            t.save()
        except:
